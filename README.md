...
- 사진 업로드 (S3 연동)
- 게시글/리뷰 구조
    - 게시글(Post)과 리뷰(Review)를 **별도 도메인/테이블**로 설계
    - 게시글은 `type` 필드로 **GENERAL / INFO / QUESTION** 등 게시글 성격을 구분
    - 이미지(Image) 엔티티/테이블은 **게시글, 리뷰, 채팅 메시지에서 공용으로 사용**할 수 있도록 분리 설계
...
- 단일 `Comment` 도메인으로 구현해 게시글/리뷰 등 다양한 콘텐츠에 공통 적용
- **좋아요 기능**
...
- 채팅방에 새 메시지가 도착하면 해당 방의 안 읽은 메시지를 기반으로 개인 알림을 생성하고,  
  사용자가 채팅방에 입장해 읽는 순간 알림 목록에서도 자동으로 제거되도록 구현
