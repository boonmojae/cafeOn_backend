package com.b1a4.cafeOn.chat.repository;

import com.b1a4.cafeOn.chat.entity.ChatEntity;
import com.b1a4.cafeOn.chat.enums.ChatMessageType;
import org.springframework.data.domain.Pageable;
import org.springframework.data.domain.Slice;

import java.time.LocalDateTime;
import java.util.List;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.stereotype.Repository;

import java.awt.*;

@Repository
public interface ChatRepository extends JpaRepository<ChatEntity, Long> {

    // 채팅방의 모든 메시지(텍스트, 이미지, 시스템)
    Slice<ChatEntity> findByChatRoom_ChatRoomIdOrderByChatIdDesc(Long roomId, Pageable pageable);

    // beforeId 기준으로 이전 메시지만 이어서 로드
    Slice<ChatEntity> findByChatRoom_ChatRoomIdAndChatIdLessThanOrderByChatIdDesc(Long roomId, Long beforeChatId, Pageable pageable);

    // 타입(TEXT, IMAGE)만 조회 시스템X
    Slice<ChatEntity> findByChatRoom_ChatRoomIdAndMessageTypeInOrderByChatIdDesc(Long chatRoomId, List<ChatMessageType> types, Pageable pageable);

    // 무한 스크롤 다음 페이지 로딩 + 타입 필터 (메시지 타입, beforeId)
    Slice<ChatEntity> findByChatRoom_ChatRoomIdAndMessageTypeInAndChatIdLessThanOrderByChatIdDesc(Long chatRoomId, List<ChatMessageType> types, Long beforeId, Pageable pageable);

    // SYSTEM_JOIN, LEAVE 메시지 중복 검사(30초 이내 재발행 차단)
    boolean existsByChatRoom_ChatRoomIdAndSender_UserIdAndMessageTypeAndCreatedAtAfter(Long roomId, String userId, ChatMessageType type, java.time.LocalDateTime after);

    // 날짜 시스템 메시지 검증
    boolean existsByChatRoom_ChatRoomIdAndMessageTypeAndCreatedAtAfter(Long roomId, ChatMessageType type, LocalDateTime after);

    // 채팅방의 최신 메시지 반환
    @Query("""
              SELECT COALESCE(MAX(c.chatId), 0)
              FROM ChatEntity c
              WHERE c.chatRoom.chatRoomId = :roomId
            """)
    Long findMaxChatIdByRoomId(@Param("roomId") Long roomId);

}
